...
def a_star(graph: MultiDiGraph, simple_graph: Dict[int, Node], source: int, destination: int, max_speed_allowed: float=100.) -> Optional[int]:
...
def run_a_star(location=None, source_point=None, destination_point=None) -> None:
  ...
  iterations = a_star(G, simple_graph, source, destination, max_speed_allowed)
  if iterations is not None:
    plot_graph(G, simple_graph, algorithm=f"a_star_assets/a_star-exploration_{iterations:08d}_2048", dpi=2048)
